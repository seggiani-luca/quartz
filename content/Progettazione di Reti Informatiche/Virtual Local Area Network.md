...
### Routing inter-VLAN
Abbiamo visto che ogni VLAN rappresenta un proprio dominio di broadcast. In questo modo, host su VLAN non sono capaci di comunicare (o almeno no possono farlo nel dominio di broadcast a livello 2 offerto dagli switch). Definiamo quindi il **routing inter-VLAN** come il processo che trasposta il traffico da una VLAN ad un altra attraverso il protocollo IP (quindi a livello 3). Questo richiede che ogni VLAN sia associata ad una sottorete IP nella rete.

- In caso di configurazioni legacy, questo veniva effettivamente fatto attraverso un router *esterno* che prevedeva una porta per VLAN, e quindi era capace di indirizzare il traffico da una VLAN all'altra. Questa configurazione era però poco efficiente in quanto un pacchetto generato da uno switch veniva inoltrato al router, per poi tornare allo switch, ma rivolto ad una VLAN diversa. Questa metodologia però non scala (un interfaccia router per VLAN è piuttosto costosa);
- Proponiamo quindi la configurazione di tipo *"router on a stick"*. In questo caso prevediamo un singolo link fisico di accesso al router (e quindi una sola porta fisica dello switch occupata). Sfruttiamo quindi le porte *virtuali*, cioè nei router CISCO le *subinterfaces*, per dichiarare più interfacce logiche che gestiscano ogni VLAN. Associando un diverso indirizzo IP ad ogni VLAN, lo switch potrà trattare questo link come un qualunque link di tipo *trunk*;
- La soluzione definitiva è chiaramente quella di uno [[Switch multilivello]], in quanto ciò che volevamo fare era *combinare* in qualche modo la funzionalità dello switch con quella del router. In questo modo il routing inter-VLAN viene fatto direttamente dallo switch, per cui pacchetti che non dovevano lasciare in primo luogo lo switch (perché un router li avrebbe reinviati al router stesso), restano nello switch.